...
def _test_boundary_base(state):
    """Use the reconciled pre-reopen target while a replacement is unanchored."""
    reopenings = state.get("test_reopenings") or []
    reopening = reopenings[-1] if reopenings else None
    if (
        reopening
...
def _approved_test_paths(root, config, state, context):
    ...
    reopenings = state.get("test_reopenings") or []
    if reopenings and reopenings[-1].get("effective_semantic") == "approved-contract-revision":
        reanchors = state.get("target_reanchors") or []
        boundary = (
            reanchors[-1].get("previous_target_head")
            if reanchors and reanchors[-1].get("new_target_head") == target_head
            else target_head
        )
        test_paths = _git_diff_names(root, config, "%s..%s" % (boundary, test_commit))
        _ensure(
            test_paths and all(_is_test_file(path) for path in test_paths),
            "test-scope-drift",
            "%s contract replacement may change only test files: %s"
            % (context, ", ".join(test_paths)),
        )
...
def command_submit_tests(args, root, config):
    ...
    if _test_reopen_active(state) and _active_test_reopening_semantic(
        state, "submit-tests"
    ) == "approved-contract-revision":
        _ensure(
            all(_is_test_file(path) for path in test_paths),
            "test-scope-drift",
    ...
    if _test_reopen_active(state) and _active_test_reopening_semantic(
        ...
        if (
            not _test_reopen_active(state)
            or _active_test_reopening_semantic(state, "submit-tests")
            != "approved-contract-revision"
        ):
            _ensure(
...
def _build_test_transition_journal(root, config, state, acknowledgment):
    ...
    if _tests_not_applicable(state):
        ...
        test_paths = _git_diff_names(
            root,
            config,
            "%s..%s" % (_test_boundary_base(state), candidate_test_commit),
        )
    ...
    if _tests_not_applicable(state):
        ...
        expected_test_paths = sorted(
            _git_diff_names(
                ...
                "%s..%s"
                % (
                    journal.get("test_boundary_base", journal["target_head"]),
                    journal["candidate_test_commit"],
                ),
            )
        )
    _ensure(
...
def _verify_authoritative_test_approval(
    ...
    if not _tests_not_applicable(state):
        ...
        _git_ancestor(
            root,
            config,
            journal.get("test_boundary_base", target_head),
            candidate_test_commit,
            context,
        )
...
def command_approve_tests(args, root, config):
    ...
    if _tests_not_applicable(state):
        ...
        if reopened_tests and _active_test_reopening_semantic(
            state, "approve-tests"
        ) == "approved-contract-revision":
            _ensure(
                all(_is_test_file(path) for path in test_paths),
...
def command_reclassify_test_reopening(args, root, config):
    """Authorize the one safe semantic change for an active reopening."""
    state = _read_state(root, config, args.issue)
    ...
    semantic = _active_test_reopening_semantic(
        state, "reclassify-test-reopening"
    )
    _ensure(
    ...
    _ensure(
        ...
        and reconciliation.get("new_target_head")
...
def command_run_validation(args, root, config):
    ...
    reopenings = state.get("test_reopenings") or []
    if (
        state.get("status") == "IMPLEMENTATION"
        and reopenings
        and reopenings[-1].get("effective_semantic") == "approved-contract-revision"
        and not reopenings[-1].get("active")
    ):
        approved_paths = _approved_test_paths(root, config, state, "run-validation")
        if approved_paths:
            test_diff = _run_bounded(
                _git_command(config, "diff", "--quiet", state["test_commit"], "--", *approved_paths),
                _effective_limits(config, "git"),
                root,
            )
            _ensure(
