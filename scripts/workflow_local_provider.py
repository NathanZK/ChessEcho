...
def _agent_prompt(issue, role, request, request_binding, inputs):
    ...
    plan_acceptance_contract = (
        ...
        "units that each contain the exact fact value. Plan unit ranges must still "
        "exhaustively tile every plan line, so treat the final three-line acceptance "
        "block as metadata, not substantive plan content, and give it its own distinct "
        "unit range. Do not include that metadata unit in any acceptance requirement's "
        "unit_ids. Every substantive mapped unit must end before the acceptance block "
        "begins and contain the exact trusted fact literal. Human-readable plan prose "
        "is allowed in addition, but it does not replace the structured block. "
        "Repeating a fact in prose does not permit omitting its structured row. Do not "
        "invent facts or weaken an `equals` assertion to `contains`. If the trusted "
        "issue body has no acceptance-facts block, do not invent structured facts or a "
        "coverage block."
        if operation == "write-plan"
        else ""
    )
