...
import argparse, base64, binascii, copy, datetime, hashlib, json, os, pathlib, re, stat, sys, tempfile
try:
...
(BOOTSTRAP_FORMAT, REQUEST_FORMAT, RESULT_FORMAT, PR_OBSERVATION_FORMAT, AUTHORIZATION_OBSERVATION_FORMAT, SANDBOX_RESULT_FORMAT, ISSUE_SNAPSHOT_FORMAT, BASELINE_FORMAT, DIFF_OBSERVATION_FORMAT, FAILURE_FORMAT) = ('chess-echo-runtime-bootstrap-v1', 'chess-echo-execution-request-v1', 'chess-echo-execution-result-v1', 'chess-echo-github-pr-observation-v1', 'chess-echo-github-authorization-observation-v1', 'chess-echo-external-sandbox-result-v1', 'chess-echo-work-type-issue-snapshot-v1', 'chess-echo-work-type-baseline-v1', 'chess-echo-work-type-diff-observation-v1', 'chess-echo-workflow-runtime-failure-v1')
...
class RuntimeFailure(Exception):
    ...
    def document(self):
        return {'format': FAILURE_FORMAT, 'outcome': {'status': self.status, 'code': self.code, 'message': self.message, **({'subject': self.subject} if self.subject is not None else {})}}
...
def _reference(value, label='binding', kind='evidence-binding'):
    ...
    if value['kind'] != kind:
        _fail('corrupt', 'invalid-%s-kind' % label, '%s kind is invalid' % label)
...
def _canonical(value):
    try:
        data = workflow_inspector.canonical_bytes(value)
    except (TypeError, ValueError, workflow_inspector.InspectionFailure) as error:
        _fail('corrupt', 'invalid-canonical-json', str(error))
    if len(data) > MAX_DOCUMENT_BYTES:
        _fail('unsupported', 'document-too-large', 'Document exceeds 2 MiB')
    return data
...
def _duplicate_rejector(pairs):
    ...
    for (key, value) in pairs:
        ...
        result[key] = value
    return result
def _parse(data, label, maximum=MAX_DOCUMENT_BYTES):
    if not isinstance(data, bytes) or len(data) > maximum:
        _fail('unsupported', '%s-too-large' % label, '%s exceeds its byte limit' % label)
    try:
        value = json.loads(data.decode('utf-8'), object_pairs_hook=_duplicate_rejector)
    except RuntimeFailure:
        raise
    except (UnicodeError, json.JSONDecodeError, RecursionError, ValueError) as error:
        _fail('corrupt', 'invalid-%s-json' % label, '%s is invalid JSON: %s' % (label, error))
    if not isinstance(value, dict):
        _fail('corrupt', 'invalid-%s-type' % label, '%s must be an object' % label)
    return value
def _path(value, root, label, allow_dot=True):
    ...
    if value.startswith('/') or '\\' in value or any((part in {'', '.', '..'} for part in value.split('/'))):
        _fail('denied', 'invalid-%s' % label, '%s must be repository-relative' % label)
    resolved = (root / value).resolve()
    try:
        resolved.relative_to(root)
    except ValueError:
        _fail('denied', '%s-escape' % label, '%s escapes the repository' % label)
    return resolved
...
def _command(value, label):
    if not isinstance(value, list) or not 1 <= len(value) <= 128:
        _fail('corrupt', 'invalid-%s' % label, '%s must be a bounded argv' % label)
    for part in value:
        _text(part, '%s-part' % label, maximum=4096)
    executable = pathlib.PurePosixPath(value[0]).name.lower()
    if executable in SHELLS or executable in WRAPPERS:
        _fail('denied', '%s-dispatch-prohibited' % label, 'Shells and dispatch wrappers are prohibited')
    return list(value)
def _executable(path, label):
    try:
        candidate = pathlib.Path(path)
        if not candidate.is_absolute():
            _fail('denied', '%s-not-absolute' % label, '%s must be absolute' % label)
        resolved = candidate.resolve(strict=True)
        metadata = resolved.stat()
        if not stat.S_ISREG(metadata.st_mode) or not os.access(resolved, os.X_OK):
            _fail('denied', '%s-not-executable' % label, '%s is not a regular executable' % label)
...
def _stdout(result, label, stream='stdout'):
    if not isinstance(result, dict) or result.get('format') != 'chess-echo-process-result-v1':
        _fail('corrupt', 'invalid-process-result', '%s returned an invalid process result' % label)
    record = result.get(stream)
    if not isinstance(record, dict) or set(record) != {'bytes', 'base64'}:
        _fail('corrupt', 'invalid-process-output', '%s output is malformed' % label)
    try:
        data = base64.b64decode(record['base64'], validate=True)
    except (TypeError, ValueError, binascii.Error):
        _fail('corrupt', 'invalid-process-output', '%s output is not strict base64' % label)
    if type(record['bytes']) is not int or record['bytes'] != len(data):
        _fail('corrupt', 'invalid-process-output-size', '%s output size is inconsistent' % label)
    return data
...
def _validate_config(config, root):
    if not isinstance(config, dict) or 'orchestrator' not in config:
        _fail('missing', 'orchestrator-config-missing', 'Base config has no orchestrator block')
    ...
    if orchestrator['format'] != 'chess-echo-orchestrator-config-v1':
        _fail('unsupported', 'orchestrator-config-format', 'Orchestrator config format is unsupported')
    if orchestrator['mode'] not in {'inactive', 'active'}:
        _fail('unsupported', 'orchestrator-mode', 'Orchestrator mode is unsupported')
    frozen = orchestrator['frozen_issues']
    if not isinstance(frozen, list) or any((type(issue) is not int or issue < 1 for issue in frozen)) or frozen != sorted(set(frozen)):
        _fail('corrupt', 'invalid-frozen-issues', 'Frozen issues must be sorted and unique')
    roles = orchestrator['agent_roles']
    if not isinstance(roles, list) or [row.get('role') for row in roles if isinstance(row, dict)] != list(ROLES):
        _fail('corrupt', 'invalid-agent-roles', 'Exactly three canonically ordered roles are required')
    for row in roles:
        ...
        if row['containment'] != 'external-sandbox-v1':
            _fail('denied', 'sandbox-containment-required', 'Agent roles require external-sandbox-v1')
    ...
    for (name, expected) in (('git', 'git'), ('github', 'gh')):
        ...
        if command != [expected]:
            _fail('denied', '%s-command-mismatch' % name, '%s command must be [%r]' % (name, expected))
    ...
    if not isinstance(paths, list) or not paths or len(paths) > 32 or (len(paths) != len(set(paths))):
        _fail('corrupt', 'invalid-validation-path', 'Validation PATH must be a bounded unique list')
    for value in paths:
    ...
    if not isinstance(accounts, list):
        _fail('corrupt', 'invalid-allowed-accounts', 'Allowed accounts must be a list')
    ids = []
    ...
    if ids != sorted(set(ids)):
        _fail('ambiguous', 'invalid-allowed-account-order', 'Allowed accounts must be sorted by unique ID')
    associations = approval['allowed_associations']
    if not isinstance(associations, list) or associations != sorted(set(associations)) or any((item not in ASSOCIATIONS for item in associations)):
        _fail('corrupt', 'invalid-allowed-associations', 'Allowed associations are invalid')
...
class Runtime:
    ...
    def __init__(self, root, repository, git_executable, gh_executable, token, config, config_bytes, document):
        ...
        self._bootstrap = document
    def __repr__(self):
        return 'Runtime(root=%r, repository=%r, mode=%r)' % (str(self.root), self.repository, self._config['mode'])
    def bootstrap_document(self):
    ...
    def _git(self, arguments, limits=None, cancel_event=None):
        ...
        result = _run([self._git_executable['path'], '-c', 'core.fsmonitor=false'] + list(arguments), limits=limits or _limits(configured, 'git'), cwd=self.root, environment=_git_env(self._paths()), cancel_event=cancel_event)
    ...
    def build_request(self, *, issue, family_run_id, attempt_id, authority_binding, operation, command_source, input_bindings, repository_before, limits, reconciliation_expectation=None):
        ...
        if operation['kind'] in {'agent', 'validation'} and repository_before is None: _fail('missing', 'repository-before-required', 'Agent and validation execution require a repository observation')
    ...
    def _validate_expectation(self, value, payload=None):
        ...
        for field in ('base_ref', 'head_ref'):
            _text(value[field], field, maximum=255)
    ...
    def execute(self, request_document, request_binding, reconciliation_expectation=None, cancel_event=None, sandbox_provider=None, write_payload=None):
        ...
        if config['mode'] != 'active':
            _fail('unsupported', 'runtime-inactive', 'Execution is disabled by base configuration')
        operation = request['operation']
        expectation = None
        if operation['kind'] == 'github-write':
            expectation = self._validate_expectation(reconciliation_expectation, write_payload)
            self._verify_tool(self._gh_executable, 'github')
        elif reconciliation_expectation is not None:
            _fail('corrupt', 'unexpected-reconciliation', 'Only GitHub writes may reconcile')
        provider_row = None
        if operation['kind'] == 'agent':
        ...
        if not preflight_cancelled:
            try:
                ...
                if before is not None and self.observe_diff(request['issue'], request['family_run_id'], before['triage_binding'], before['observed_at'], cancel_event) != before: _fail('stale', 'repository-before-mismatch', 'Current repository differs from request observation')
            except RuntimeFailure:
        ...
        process = _run(command, limits=request['limits'], cwd=cwd, environment=environment, cancel_event=cancel_event)
        if operation['kind'] == 'github-write': self._token_safe(process)
        candidate = _stdout(process, 'execution')
        process_ok = process.get('outcome') == 'success' and process.get('exit_code') == 0 and (process.get('cleanup_verified') is True)
        reconciliation = {'status': 'not-required', 'external_identity': None}
        outcome = 'succeeded' if process_ok else 'failed'
        if process.get('reason') in {'cancelled', 'cancelled-before-start'}:
            outcome = 'cancelled'
        if operation['kind'] == 'github-write' and not process_ok and process.get('reason') != 'cancelled-before-start':
            (reconciliation, outcome) = self._reconcile_pr(expectation, cancel_event)
        ...
        if request['repository_before'] is not None:
            if cancel_event is not None and cancel_event.is_set(): outcome = 'cancelled'
            else:
                try: repository_after = self.observe_diff(request['issue'], request['family_run_id'], request['repository_before']['triage_binding'], cancel_event=cancel_event)
                except RuntimeFailure as failure:
                    if cancel_event is not None and cancel_event.is_set(): outcome = 'cancelled'
                    elif failure.code == 'document-too-large': outcome = 'failed'
                    else: raise
            if repository_after is None and outcome == 'succeeded': outcome = 'cancelled' if cancel_event is not None and cancel_event.is_set() else 'failed'
        if cancel_event is not None and cancel_event.is_set() and outcome == 'succeeded': outcome = 'cancelled'
    ...
    def _reconcile_pr(self, expectation, cancel_event):
        ...
        result = self._github(['api', '--paginate', '--slurp', 'repos/%s/pulls?state=open&per_page=100&base=%s&head=%s%%3A%s' % (self.repository, _urlencode(expectation['base_ref']), _urlencode(owner), _urlencode(expectation['head_ref']))], cancel_event=cancel_event, allow_failure=True)
        try:
            data = _stdout(result, 'github-reconciliation')
            values = json.loads(data.decode('utf-8'))
        except (RuntimeFailure, UnicodeError, json.JSONDecodeError):
            return ({'status': 'unknown', 'external_identity': None}, 'uncertain')
        if result.get('outcome') != 'success' or not isinstance(values, list) or any(not isinstance(page, list) for page in values):
            return ({'status': 'unknown', 'external_identity': None}, 'uncertain')
    ...
    def _pr_matches(self, value, expected):
        try:
            return value['state'] == 'open' and value['draft'] is True and (value['base']['ref'] == expected['base_ref']) and (value['base']['sha'] == expected['base_sha']) and (value['head']['ref'] == expected['head_ref']) and (value['head']['sha'] == expected['head_sha']) and (workflow_inspector.sha256(value['title'].encode()) == expected['title_sha256']) and (workflow_inspector.sha256((value.get('body') or '').encode()) == expected['body_sha256']) and value['html_url'].startswith('https://github.com/%s/pull/' % self.repository)
...
def _parse_raw_diff(data, oid_length):
    ...
    while index < len(parts) and parts[index]:
        try:
            ...
            _fail('corrupt', 'invalid-raw-diff', 'Git raw diff metadata is invalid')
        if len(metadata) != 5 or not metadata[0].startswith(':'):
            _fail('corrupt', 'invalid-raw-diff', 'Git raw diff shape is invalid')
        (old_mode, new_mode) = (metadata[0][1:], metadata[1])
        (old_oid, new_oid, change_status) = (metadata[2], metadata[3], metadata[4])
        if change_status not in {'A', 'D', 'M', 'T'} or index + 1 >= len(parts):
            _fail('unsupported', 'unsupported-raw-diff', 'Git diff status is unsupported')
        path = _decode_path(parts[index + 1], 'diff-path')
        _oid(old_oid, 'old-object', oid_length)
        _oid(new_oid, 'new-object', oid_length)
...
def _parse_status(data):
    ...
    while index < len(parts) and parts[index]:
        row = parts[index]
        if len(row) < 4:
            _fail('corrupt', 'invalid-workspace-status', 'Git status output is malformed')
        status = row[:2].decode('ascii')
        path = _decode_path(row[3:], 'workspace-path')
        if status == '??':
            workspace['untracked_non_ignored'].append(path)
        elif status == '!!':
            pass
        else:
            original = None
            if status[0] in {'R', 'C'} or status[1] in {'R', 'C'}:
                index += 1
                if index >= len(parts):
                    _fail('corrupt', 'invalid-workspace-rename', 'Git rename record is incomplete')
                original = _decode_path(parts[index], 'workspace-original-path')
            if status[0] not in {' ', '?', '!'}:
                workspace['staged'].append({'code': status[0], 'path': path, 'original_path': original})
            if status[1] not in {' ', '?', '!'}:
                workspace['unstaged'].append({'code': status[1], 'path': path, 'original_path': original})
        index += 1
...
def _profiles(config):
    ...
    if not isinstance(profiles, dict) or set(profiles) != set(expected):
        _fail('corrupt', 'invalid-validation-profiles', 'Validation profiles are incomplete')
    result = []
    for profile_id in sorted(expected):
        profile = profiles[profile_id]
        if not isinstance(profile, dict) or set(profile) != {'checks', 'test_paths'}:
            _fail('corrupt', 'invalid-validation-profile', 'Validation profile schema is invalid')
        checks = []
        names = set()
        for raw in profile['checks']:
            if not isinstance(raw, dict) or set(raw) not in ({'name', 'command'}, {'name', 'command', 'cwd'}):
                _fail('corrupt', 'invalid-validation-check', 'Validation check schema is invalid')
            name = _slug(raw['name'], 'validation-check-name')
            if name in names:
                _fail('ambiguous', 'duplicate-validation-check', 'Validation check is duplicated')
            names.add(name)
        ...
        if not isinstance(paths, list) or not paths or any((not isinstance(item, str) or not item for item in paths)):
            _fail('corrupt', 'invalid-validation-test-paths', 'Validation test paths are invalid')
...
def bootstrap(root, repository, git_executable, gh_executable, github_token):
    try:
        ...
        _fail('missing', 'repository-root-unavailable', 'Repository root is unavailable: %s' % error)
    if not root.is_dir() or not isinstance(repository, str) or REPOSITORY_RE.fullmatch(repository) is None:
        _fail('corrupt', 'invalid-bootstrap-target', 'Bootstrap root or repository is invalid')
