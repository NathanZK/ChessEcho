...
The experiment used the existing BFS endpoint/request pipeline and added temporary runtime instrumentation to `src/main/kotlin/com/chessecho/service/HumanMoveBfsService.kt`.

Instrumentation validation tests are in `src/test/kotlin/com/chessecho/service/HumanMoveBfsServiceTest.kt`.

The experiment was invoked through the existing BFS request endpoint and request configuration (`HumanMoveBfsController` and `HumanMoveBfsRequest`).
...
This report records the evidence needed to evaluate current batch-threshold behavior and inform subsequent architectural decisions.
